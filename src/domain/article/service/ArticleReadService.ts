...
export class ArticleReadService {
    ...
    async getArticlesByCategoryIds(article_page: number, category_id: string, isAdmin: boolean): Promise<{ articles: Article[], totalPage: number }> {
        const pageSize = 8;
        try {
            const queryBuilder = this.articleRepository.createQueryBuilder('article')
                .leftJoinAndSelect('article.categorys', 'category')
                .where('category.id = :categoryId', { categoryId: category_id });
